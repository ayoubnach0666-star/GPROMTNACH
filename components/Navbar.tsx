...
import { Search, Zap, LayoutGrid, ShieldCheck } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ searchQuery, setSearchQuery, isAdmin, onJoinClick }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const isAtAdmin = location.pathname === '/admin';
  ...
  return (
    <nav className="sticky top-0 z-50 glass px-6 py-5">
      <div className="max-w-[1400px] mx-auto grid grid-cols-2 md:grid-cols-3 items-center gap-4">
        ...
        {/* Search Bar - Disabled on Admin Page to focus on form */}
        <div className="relative group hidden md:block">
          {!isAtAdmin ? (
            ...
            <div className="flex justify-center items-center gap-3 text-[#FF9F1C]/40">
              <ShieldCheck size={16} />
              <span className="text-[10px] font-black uppercase tracking-[0.3em]">Secure Administrative Channel</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3 md:gap-6">
          {isAdmin && (
            <button 
              onClick={() => navigate(isAtAdmin ? '/' : '/admin')}
              className={`p-3 rounded-xl transition-all active:scale-90 border ${
                isAtAdmin 
                ? 'bg-zinc-900 border-zinc-800 text-zinc-400' 
                : 'bg-[#FF9F1C]/10 border-[#FF9F1C]/20 text-[#FF9F1C]'
              }`}
              title={isAtAdmin ? "Back to Gallery" : "Admin Panel"}
            >
              <LayoutGrid size={20} />
            </button>
          )}
